...
| OCR golden-set evaluation | `npm run eval` | **PASS on synthetic fixtures** — first run 2026-09-03 through the deployed worker proxy: 3/3 samples scored, the blank-paper negative returned zero medicines (no hallucination), and the 2100px page plus its 1568px-downscaled twin both scored 100% on name/dosage/frequency/duration. Synthetic typed pages only — real anonymized prescriptions remain the prerequisite for a meaningful accuracy figure |
...
- **Eval accuracy figures rest on synthetic fixtures only.** The harness ran for the first time on 2026-09-03 (3 samples, through the deployed worker): the blank-paper negative returned zero medicines, and `synthetic_typed_rx` plus its 1568px-downscaled twin both scored 100% on every field — so downscaling the upload copy to `OCR_MAX_IMAGE_EDGE` costs nothing on clean typed print. That is not a real-world accuracy number: the golden set still holds no photographed, handwritten or glare-affected prescription, and `eval/results.csv` holds a single run. It is not part of CI. Populating `eval/golden/` with 20-40 anonymized real prescriptions remains the prerequisite for measuring OCR quality.
